    ...
    for (int y = 0; y < numSegments; ++y) {
        for (int x = 0; x < numSegments; ++x) {
            for (int i = 0; i < 2; ++i) {
                ...
                auto segment = segments[i * 3 + (i == 0 ? 0 : 3)]; // Select the right segment for each triangle
...
void generatePhongCylinderVertices(float radius, float height, int numSegments, std::vector<float>& vertices) { // generated by chatgpt
    ...
    for (int i = 0; i < numSegments; i++) {
        ...
        std::vector<float> quadVertices = {
            // First triangle
            x_bottom, y_bottom, z_bottom, nx, 0.0f, nz, static_cast<float>(i) / numSegments, 1.0f,
            x_top, y_top, z_top, nx, 0.0f, nz, static_cast<float>(i) / numSegments, 0.0f,
            x_top_next, y_top, z_top_next, cos(nextTheta), 0.0f, sin(nextTheta), static_cast<float>(i + 1) / numSegments, 0.0f,

            // Second triangle
            x_bottom, y_bottom, z_bottom, nx, 0.0f, nz, static_cast<float>(i) / numSegments, 1.0f,
            x_top_next, y_top, z_top_next, cos(nextTheta), 0.0f, sin(nextTheta), static_cast<float>(i + 1) / numSegments, 0.0f,
            x_bottom_next, y_bottom, z_bottom_next, cos(nextTheta), 0.0f, sin(nextTheta), static_cast<float>(i + 1) / numSegments, 1.0f,
        };
